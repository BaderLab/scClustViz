...
#'   input file.
#'
#' @param annotationDB Optional. An \code{AnnotationDb}-derived object
#'   containing identifier mappings for the species in the data. Used to search
#'   for genes if rownames are not converted to gene symbols, and for searching
#'   by and displaying gene names in the app. See examples.
#'
#' @param cellMarkers Optional. If you have canonical marker genes for expected
#'   cell types, list them here (see example code below). The Shiny app will
#'   attempt to label clusters in the tSNE projection by highest median gene
#'   expression. Optional.
#'
...
#'   This is used if you use the function for testing differential gene
#'   expression between custom sets, and should be set to the same parameters as
#'   in \code{clusterWiseDEtest}.
#'
#' @param FDRthresh The false discovery rate to use as a threshold for determining
#'   statistical significance of differential expression calculated by the
#'   Wilcoxon rank-sum test. This is used if you use the function for testing
...
#'   can set threshType to \code{"logGER"} to use a gene expression ratio for
#'   all gene filtering, or leave it as default (\code{"dDR"}) to use difference
#'   in detection rate as the thresholding method when comparing clusters to
#'   each other. This is used if you use the function for testing differential
#'   gene expression between custom sets, and should be set to the same
#'   parameters as in \code{clusterWiseDEtest}.
#'
...
#'   differential expression between clusters. This is used if you use the
...
runShiny <- function(filePath,annotationDB,cellMarkers=list(),
  ...
  ui <- fixedPage(
    ...
    fixedRow(
      ...
      p(HTML(paste0("Here you can explore the significantly differentially expressed genes per",
  ...
  server <- function(input,output,session) {
    ...
    plot_heatmap <- function() {
      if (length(levels(clusts())) <= 1) {
        ...
        gplots::heatmap.2(clustMeans(),Rowv=as.dendrogram(hC()),Colv=as.dendrogram(hG()),scale="column",
    ...
    output$geneSearchBox <- renderUI({
      if (input$searchType == "comma") {
        ...
        textInput("GOI",value="^ACTB$",width="100%",
    ...
    plot_geneTest <- function() {
      if (input$cgGene == "") {
        ...
        try(tempGeneName <- select(annotationDB,keys=input$cgGene,
    ...
    plot_goi <- function(goi) {
      if (length(goi) < 1) {
        ...
        try(tempGeneName <- 
              select(annotationDB,keys=goi,keytype="SYMBOL",column="GENENAME")$GENENAME,silent=T)
        if (exists("tempGeneName")) { 
