...
#'   been changed, the objects can be named anything you'd like. Note that any files
#'   generated by the Shiny app (ie. saving the selected cluster solution, saving custom
#'   set DE testing results) will be saved/loaded in the same directory as the input file.
#'
#' @param cellMarkers Optional. If you have canonical marker genes for expected cell
#'   types, list them here (see example code below). Note that the gene names must match
#'   rownames of your data (ie. use ensembl IDs if your gene expression matrix rownames
#'   are ensembl IDs). The Shiny app will attempt to label clusters in the tSNE projection
#'   by highest median gene expression.
#'
#' @param annotationDB Optional. If the gene IDs in your data aren't official gene
#'   symbols, you may want to pass an AnnotationDbi object (ie. org.Mm.eg.db /
#'   org.Hs.eg.db for mouse / human respectively) as this argument. This will allow the
#'   Shiny interface to both display gene symbols when highlighting genes in figures, and
#'   search by gene symbol.
#'
#' @param rownameKeytype Optional. If passing the annotationDB argument, it helps if you
#'   indicate what keytype (see \code{AnnotationDbi::keytypes(annotationDB)}) the rownames
#'   of your data are. If this is missing, it will be determined automatically, but that
#'   will take about 30s.
#'
...
#'   statistical significance of differential expression calculated by the Wilcoxon
#'   rank-sum test. This is used if you use the function for testing differential gene
...
#'                                     "Nes","Cux1","Cux2"),
#'             "Interneurons"=c("Gad1","Gad2","Npy","Sst",
#'                              "Lhx6","Tubb3","Rbfox3","Dcx"),
#'             "Cajal-Retzius neurons"="Reln",
#'             "Intermediate progenitors"="Eomes",
#'             "Projection neurons"=c("Tbr1","Satb2","Fezf2","Bcl11b",
#'                                    "Tle4","Nes","Cux1","Cux2",
#'                                    "Tubb3","Rbfox3","Dcx")
#'           )
#'  )
...
runShiny <- function(filePath,cellMarkers=list(),
