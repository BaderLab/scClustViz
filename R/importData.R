...
#'   interpretation). Will perform a one-to-one mapping where possible using the 
#'   \code{AnnotationDbi} Bioconductor package and provided database (see arguments).
#'
#' @param annotationDB An \code{AnnotationDb}-derived object containing identifier 
#'   mappings for the species in the data. See examples. (Only required if 
#'   \code{convertGeneIDs = TRUE}).
#'
#' @param geneRowNames The column name (\code{see AnnotationDbi::columns()}) containing 
#'   the current gene ID type (rownames) of the data. (Only required if 
#'   \code{convertGeneIDs = TRUE}).
#'   
...
readFromSeurat <- function(inD,convertGeneIDs=FALSE,
                           annotationDB=NULL,geneRowNames=NULL) {
  ...
  if (convertGeneIDs) {
    ...
    if (missing(annotationDB)) {
      stop("The argument 'annotationDB' must be a loaded AnnotationDbi database.")
    }
    if (! geneRowNames %in% AnnotationDbi::columns(annotationDB)) {
      stop("geneRowNames must refer to a column of the provided AnnotationDbi database.")
    }
    x2g <- mapIds(get(annotationDB),test,"SYMBOL",geneRowNames,multiVals="first")
    x2g <- x2g[!is.na(x2g)]
    #### FIX THIS ####
