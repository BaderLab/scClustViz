...
if (!file.exists("../yourData/output/yourData_eb1S.RData")) {
  ...
  while (minDEgenesToNeighbour > 1) {
    ...
    for (i in seq_along(deGall)) {
      deGall[[i]]$fwer <- p.adjust(deGall[[i]]$p_val,"holm")
      deGall[[i]] <- deGall[[i]][deGall[[i]]$fwer <= 1e-2 & deGall[[i]]$avg_diff > 0,] # postive DE at FWER of 1%
    }
    ...
    for (i in seq_along(deGvs)) {
      deGvs[[i]]$fwer <- p.adjust(deGvs[[i]]$p_val,"holm")
      deGvs[[i]]$clustPair <- factor(compClust[i])
      deGvs[[i]]$gene <- rownames(deGvs[[i]])
      deGvs[[i]] <- deGvs[[i]][deGvs[[i]]$fwer <= 5e-2,] # FWER at 5%
    }
    ...
    print(summary(minDEgenesToNeighbour[pw]))
