...
temp_cells <- scan("yourData_CELLNAMES.txt",character(),sep="\t") #column names
temp_genes <- scan("yourData_GENENAMES.txt",character(),sep="\t") #row names
...
if (!file.exists("../yourData/output/yourData_eb1S.RData")) {
  ...
  rm(eb1F)
  gc()
  eb1Ssm <- eb1S <- Setup(eb1S,"170402_CC",min.cells=0,min.genes=0,do.logNormalize=F,save.raw=F)
  ...
  while (minUniqDE > 1) {
    ...
    for (i in seq_along(deGall)) {
      ...
      deGall[[i]] <- deGall[[i]][deGall[[i]]$fdr <= 5e-2 & deGall[[i]]$avg_diff > 0,] # postive DE at FDR of 5%
    }
    ...
    for (i in seq_along(deGvs)) {
      ...
      deGvs[[i]] <- deGvs[[i]][deGvs[[i]]$fdr <= 5e-2,] # FDR at 5%
    }
    ...
    uniqDE[[res]] <- tapply(deGvs$gene,deGvs$posClust,function(X) 
      names(which(table(X) == length(levels(deGvs$posClust))-1)))
    
    print("Number of unique DE genes per cluster:")
    ...
    save(deGvs,file=paste0("../yourData/output/yourData_",gsub(".","",res,fixed=T),"_deGvs.RData"))
    save(deGall,file=paste0("../yourData/output/yourData_",gsub(".","",res,fixed=T),"_deGall.RData"))
    save(CGS,file=paste0("../yourData/output/yourData_",gsub(".","",res,fixed=T),"_CGS.RData"))
