...
if (!file.exists("Fig2data.RData")) {
  ...
  rownames(deM_dDR) <- rownames(dd$CGS[[res]][["1"]])
  deM_logGER <- apply(combos,2,function(i) dd$CGS[[res]][[i[1]]]$MTC - dd$CGS[[res]][[i[2]]]$MTC)
  rownames(deM_logGER) <- rownames(dd$CGS[[res]][["1"]])
  ...
  save(dDR,GER,dDR2,GER2,file="Fig2data.RData")
} else {
  load("Fig2data.RData")
}
######## Combined Figure ########
...
setEPS()
postscript(file="Fig2.eps",width=6,height=6)
